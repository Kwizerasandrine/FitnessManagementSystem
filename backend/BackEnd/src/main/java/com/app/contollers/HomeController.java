...
public class HomeController {
	...
	public ResponseEntity<?> login(@RequestBody Credentials cred) {
		...
		if (tu != null) {
			// Generate 2FA code
			userService.generateTwoFactorCode(tu.getEmail());
			return Response.success("2FA_REQUIRED");
		} else {
			return Response.error("error");
		}
	}
	
	@RequestMapping("/verify2fa")
	public ResponseEntity<?> verify2fa(@RequestBody Credentials cred) {
		User tu = userService.verifyTwoFactorCode(cred);
		Credentials usercred = new Credentials();
